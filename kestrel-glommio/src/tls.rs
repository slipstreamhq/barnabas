//! TLS over glommio, as a second [`Transport`](kestrel_client::Transport).
//!
...
impl kestrel_client::Transport for GlommioTls {
    ...
    async fn connect(&self, addr: &str) -> io::Result<Self::Stream> {
        ...
        let tcp = <Glommio as kestrel_client::Transport>::connect(&Glommio, addr).await?;
...
pub type TlsCluster = kestrel_client::Cluster<GlommioTls>;
/// An assign-only consumer over TLS.
pub type TlsConsumer = kestrel_client::Consumer<GlommioTls>;
/// A producer over TLS.
pub type TlsProducer = kestrel_client::Producer<GlommioTls>;
