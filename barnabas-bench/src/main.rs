...
//! throughput and latency — and prints `barnabas` beside `rdkafka` for each.
//!
...
//!
//! ```sh
//! cargo run -p barnabas-bench --release
//! ```
...
fn consume_records() -> usize {
    std::env::var("BARNABAS_CONSUME_RECORDS")
        .ok()
...
/// The bootstrap list as barnabas wants it: one address per element.
///
/// `KAFKA_BOOTSTRAP` is a comma-separated string because that is what
/// librdkafka takes; handing the whole string to barnabas as a single address
/// would produce one unresolvable host.
fn brokers() -> Vec<String> {
    broker().split(',').map(|s| s.trim().to_owned()).collect()
}

/// Which sections to run, as a comma-separated list in `BARNABAS_CELLS`
...
fn want(section: &str) -> bool {
    match std::env::var("BARNABAS_CELLS") {
...
fn barnabas_produce(topic: &str, cell: &Cell) -> Duration {
    ...
    runtime.block_on(async {
        let mut producer =
            barnabas_tokio::Producer::idempotent(barnabas_tokio::Tokio, &brokers(), "bench")
                .await
                .expect("producer");
...
fn barnabas_produce_one_partition(topic: &str, cell: &Cell) -> Duration {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .expect("runtime");
...
fn barnabas_latency(topic: &str, samples: usize, value_bytes: usize) -> Latency {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .expect("runtime");
...
fn barnabas_consume(topic: &str, expect: usize) -> Duration {
    ...
    glommio::LocalExecutorBuilder::default()
        ...
        .run(async move {
            ...
            let mut consumer = barnabas_glommio::Consumer::new(
                barnabas_glommio::Glommio,
                &brokers(),
                "bench",
                barnabas_core::IsolationLevel::ReadCommitted,
            )
            .await
            .expect("consumer");
            for partition in 0..PARTITIONS {
                consumer
                    .assign(&topic, partition, barnabas_glommio::EARLIEST)
                    .await
                    .expect("assign");
            }
            ...
            assert_eq!(seen, expect, "barnabas consumed {seen} of {expect}");
            if std::env::var("BARNABAS_DEBUG").is_ok() {
                eprintln!(
                    "  barnabas: {polls} polls, {:.0} records/poll",
...
fn barnabas_produce_many_cores(topic: &str, cores: usize, cell: &Cell) -> Duration {
    assert!(
        ...
        .map(|core| {
            ...
            std::thread::spawn(move || {
                glommio::LocalExecutorBuilder::new(glommio::Placement::Fixed(core))
                    ...
                    .expect("executor")
                    .run(async move {
                        let mut producer = barnabas_glommio::Producer::idempotent(
                            barnabas_glommio::Glommio,
                            &brokers(),
                            "bench",
                        )
...
fn barnabas_produce_compressed(topic: &str, cell: &Cell, codec: CompressionCodec) -> Duration {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .expect("runtime");
...
/// never have two.
fn barnabas_produce_pipelined(topic: &str, cell: &Cell, in_flight: usize) -> Duration {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .expect("runtime");
...
fn barnabas_end_to_end(topic: &str, samples: usize, value_bytes: usize, prefetch: bool) -> Latency {
    ...
    glommio::LocalExecutorBuilder::default()
        ...
        .run(async move {
            ...
            let mut consumer = barnabas_glommio::Consumer::for_partition(
                barnabas_glommio::Glommio,
                &brokers(),
                "bench-e2e-consumer",
                &topic,
                0,
                barnabas_glommio::LATEST,
                barnabas_core::IsolationLevel::ReadCommitted,
            )
            .await
            .expect("consumer");
...
fn rdkafka_end_to_end(topic: &str, samples: usize, value_bytes: usize) -> Latency {
    ...
    runtime.block_on(async {
        ...
        for i in 0..samples {
            ...
            // exactly what it did. The barnabas side pins partition 0 too, so
...
/// unmeasured.
fn barnabas_idle_polls(topic: &str, partitions: i32, polls: usize, incremental: bool) -> Duration {
    let topic = topic.to_owned();
    glommio::LocalExecutorBuilder::default()
        .make()
        .expect("executor")
...
/// `BARNABAS_SOAK_SECONDS` because it is slow by construction.
fn barnabas_soak(topic: &str, seconds: u64) {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .expect("runtime");
    ...
    runtime.block_on(async {
        let mut producer =
            barnabas_tokio::Producer::idempotent(barnabas_tokio::Tokio, &brokers(), "bench-soak")
                .await
                .expect("producer");
...
/// The same shape as [`barnabas_produce_many_cores`], for librdkafka: **N
...
fn barnabas_consume_tokio(topic: &str, expect: usize) -> Duration {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .expect("runtime");
    ...
    runtime.block_on(async {
        let mut consumer = barnabas_tokio::Consumer::new(
        ...
        for partition in 0..PARTITIONS {
            consumer
                .assign(topic, partition, barnabas_tokio::EARLIEST)
                .await
                .expect("assign");
        }
        consumer.set_max_wait(Duration::from_millis(100));
        ...
        assert_eq!(seen, expect, "barnabas/tokio consumed {seen} of {expect}");
        start.elapsed()
    })
}
...
fn barnabas_consume_per_partition(topic: &str, expect: usize) -> Duration {
    let topic = topic.to_owned();
    glommio::LocalExecutorBuilder::default()
        .make()
        .expect("executor")
...
fn main() {
    ...
    if want("produce") {
        println!(
            "produce throughput      barnabas     1-partition      rdkafka      rskafka   vs rsk"
        );
        let cells = [
        ...
        for cell in &cells {
            ...
            let barnabas = cell.rate(barnabas_produce(&barnabas_topic, cell));
            ...
            let one_partition = if fits {
                ...
                cell.rate(barnabas_produce_one_partition(&one_topic, cell))
            } else {
                f64::NAN
            };
            ...
            println!(
                ...
                cell.batch, cell.value_bytes, barnabas, one_partition, rdkafka, rskafka, ratio
            );
        }
    }

    if want("latency") {
        println!("\nproduce latency (1 record)      barnabas        rdkafka");
        let samples = 2_000;
        let barnabas_topic = topic("kl");
        create_topic(&barnabas_topic);
        let mut barnabas = barnabas_latency(&barnabas_topic, samples, 128);
    ...
    if want("cores") {
        ...
        for cores in [1usize, 2, 4, 8] {
            let topic = topic("mc");
            create_topic(&topic);
            let rate = cell.rate(barnabas_produce_many_cores(&topic, cores, &cell));
            if cores == 1 {
                single_core = rate;
            }
    ...
    if want("consume") {
        ...
        create_topic(&barnabas_topic);
        barnabas_produce(&barnabas_topic, &cell);
        let barnabas_rate = cell.rate(barnabas_consume(&barnabas_topic, cell.records));
        ...
        barnabas_produce(&pp_topic, &cell);
        let pp_rate = cell.rate(barnabas_consume_per_partition(&pp_topic, cell.records));
        println!("      (barnabas, one consumer per partition: {pp_rate:.0} rec/s)");

        let tk_topic = topic("tc");
        create_topic(&tk_topic);
        barnabas_produce(&tk_topic, &cell);
        let tokio_rate = cell.rate(barnabas_consume_tokio(&tk_topic, cell.records));
        println!("      (barnabas on tokio: {tokio_rate:.0} rec/s)");

        let rs_topic = topic("sc");
        create_topic(&rs_topic);
        barnabas_produce(&rs_topic, &cell);
        let rskafka_rate = cell.rate(rskafka_consume(&rs_topic, cell.records));

        let ratio = barnabas_rate / rskafka_rate;
        let marker = if barnabas_rate < rdkafka_rate || barnabas_rate < rskafka_rate {
            "  <-- slower"
        } else {
            ""
        };
        println!(
            ...
            cell.value_bytes, barnabas_rate, rdkafka_rate, rskafka_rate, ratio
        );
    ...
    if want("e2e") {
        ...
        println!("           barnabas   no prefetch       rdkafka");
        let samples = 500;
        let barnabas_topic = topic("ke");
        create_topic(&barnabas_topic);
        let mut barnabas = barnabas_end_to_end(&barnabas_topic, samples, 128, true);
        ...
        let mut no_prefetch = barnabas_end_to_end(&plain_topic, samples, 128, false);
    ...
    if want("pipeline") {
        ...
        for window in [1usize, 100, 1_000] {
            let topic = topic("p1");
            create_topic(&topic);
            let rate = one.rate(barnabas_produce_pipelined(&topic, &one, window));
        ...
        for in_flight in [1usize, 2, 5, 10] {
            let topic = topic("pl");
            create_topic(&topic);
            let rate = cell.rate(barnabas_produce_pipelined(&topic, &cell, in_flight));
            if in_flight == 1 {
                single = rate;
            }
            println!(
    ...
    if want("idle") {
        ...
        let polls = 2_000;
        let full = polls as f64 / barnabas_idle_polls(&idle_topic, 64, polls, false).as_secs_f64();
        let incremental =
            polls as f64 / barnabas_idle_polls(&idle_topic, 64, polls, true).as_secs_f64();
    ...
    // ── soak ────────────────────────────────────────────────────────────────
    if let Some(seconds) = std::env::var("BARNABAS_SOAK_SECONDS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
    {
        ...
        barnabas_soak(&soak_topic, seconds);
    }
}
