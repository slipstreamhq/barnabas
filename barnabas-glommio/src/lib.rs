...
//! [`barnabas_client`], written once and shared with the tokio binding. This
...
//! property of *this* binding, not of the client: `barnabas-client` places no
...
impl barnabas_client::Transport for Glommio {
...
pub type Cluster = barnabas_client::Cluster<Glommio>;

/// An assign-only consumer on this core.
pub type Consumer = barnabas_client::Consumer<Glommio>;

/// An idempotent, optionally transactional producer on this core.
pub type Producer = barnabas_client::Producer<Glommio>;

/// The admin client on glommio. See [`barnabas_client::admin`].
pub type Admin = barnabas_client::Admin<Glommio>;
