...
fn a_single_member_is_assigned_every_partition() {
    run(|| async {
        ...
        let mut cluster =
            barnabas_glommio::Cluster::connect(barnabas_glommio::Glommio, &bootstrap(), "barnabas-test")
                .await
                .expect("cluster");
...
fn a_committed_offset_is_where_the_next_consumer_resumes() {
    run(|| async {
        ...
        let first = {
            ...
            consumer
                .subscribe(&group, vec![topic.clone()], Box::new(RangeAssignor), barnabas_glommio::EARLIEST)
                .await
                .expect("subscribe");
...
fn heartbeating_without_polling_keeps_membership() {
    run(|| async {
        ...
        consumer
            .subscribe(&unique("hbg"), vec![topic.clone()], Box::new(RangeAssignor), barnabas_glommio::EARLIEST)
            .await
            .expect("subscribe");
