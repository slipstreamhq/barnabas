...
use barnabas_client::group::{ClassicProtocol, GroupProtocol, Membership};
use barnabas_core::group::RangeAssignor;
...
async fn settle(
    ...
    cluster: &mut barnabas_glommio::Cluster,
) -> Vec<barnabas_core::group::TopicPartition> {
    for _ in 0..200 {
...
async fn a_single_member_is_assigned_every_partition() {
    ...
    let mut protocol =
        ClassicProtocol::new(unique("g"), vec![topic.clone()], Box::new(RangeAssignor));
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn two_members_split_the_partitions_without_overlap() {
    ...
    let mut cluster_a =
        barnabas_glommio::Cluster::connect(barnabas_glommio::Glommio, &bootstrap(), "member-a")
            .await
            .expect("cluster a");
    let mut cluster_b =
        barnabas_glommio::Cluster::connect(barnabas_glommio::Glommio, &bootstrap(), "member-b")
            .await
            .expect("cluster b");
    ...
    for _ in 0..60 {
        let (ra, rb) =
            futures_lite::future::zip(a.advance(&mut cluster_a), b.advance(&mut cluster_b)).await;
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn a_committed_offset_is_where_the_next_consumer_resumes() {
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn a_rebalance_revokes_assigns_and_auto_commits() {
    ...
    impl barnabas_glommio::RebalanceListener for Recorder {
    ...
    let mut first = barnabas_glommio::Consumer::new(
        barnabas_glommio::Glommio,
        &bootstrap(),
        "member-1",
        barnabas_core::IsolationLevel::ReadCommitted,
    ...
    // to complete.
    let mut second = barnabas_glommio::Consumer::new(
        barnabas_glommio::Glommio,
        &bootstrap(),
        "member-2",
        barnabas_core::IsolationLevel::ReadCommitted,
    ...
    let mut third = barnabas_glommio::Consumer::new(
        barnabas_glommio::Glommio,
        &bootstrap(),
        "member-3",
        barnabas_core::IsolationLevel::ReadCommitted,
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn heartbeating_without_polling_keeps_membership() {
    let topic = unique("hb");
    let mut prod = TestProducer::connect(&broker(), &topic).await;
...
async fn cooperative_rebalancing_never_drops_everything() {
    use std::cell::Cell;
    use std::rc::Rc;
    ...
    async fn member(
        ...
        consumer
            .subscribe(
                &group,
                vec![topic],
                Box::new(barnabas_core::group::CooperativeStickyAssignor),
                barnabas_glommio::EARLIEST,
            )
            .await
            .expect("subscribe");
    ...
    assert_eq!(
        a + b,
        4,
        "every partition must be held by exactly one: {a} + {b}"
    );
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn committed_reports_only_what_was_committed() {
    ...
    let mut consumer = barnabas_glommio::Consumer::new(
        ...
        &bootstrap(),
        "committed-1",
        barnabas_core::IsolationLevel::ReadUncommitted,
    )
    .await
    .expect("consumer");
    consumer.set_max_wait(Duration::from_millis(100));
    ...
    let before = consumer
        .committed(std::slice::from_ref(&p0))
        .await
        .expect("committed");
