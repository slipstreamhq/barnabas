...
#[allow(dead_code)]
#[glommio::main]
async fn readme_consume() {
    let mut consumer = Consumer::new(Glommio, &brokers(), "my-app", IsolationLevel::ReadCommitted)
        .await
        .expect("consumer");

    for partition in 0..8 {
        consumer
            .assign("events", partition, EARLIEST)
            .await
            .expect("assign");
    }
...
#[allow(dead_code)]
#[glommio::main]
async fn readme_produce_and_transactions() {
...
#[allow(dead_code)]
#[glommio::main]
async fn readme_sasl() {
    let mut cluster = barnabas_glommio::Cluster::connect(Glommio, &brokers(), "my-app")
        .await
        .expect("cluster");
...
#[allow(dead_code)]
#[glommio::main]
async fn readme_builder() {
    use barnabas_glommio::StartOffset;
    ...
    let _producer = Producer::builder(Glommio)
        ...
        .compression(barnabas_glommio::CompressionCodec::Snappy)
        .max_in_flight(5)
        .build()
        .await
        .expect("producer");
...
#[allow(dead_code)]
#[glommio::main]
async fn readme_exactly_once_with_a_group() {
    let mut consumer = Consumer::new(Glommio, &brokers(), "my-app", IsolationLevel::ReadCommitted)
        .await
        .expect("consumer");
    let mut producer = Producer::transactional(Glommio, &brokers(), "my-app", "my-app-sink-0")
        .await
        .expect("producer");
...
#[allow(dead_code)]
#[glommio::main]
async fn readme_pausing_and_offsets() {
    use barnabas_core::group::TopicPartition;
...
#[allow(dead_code)]
#[glommio::main]
async fn readme_admin() {
    use barnabas_core::group::TopicPartition;
    use barnabas_glommio::{Admin, NewTopic};
    ...
    admin
        .create_topics(&[NewTopic::new("events", 8, 3).with_config("retention.ms", "604800000")])
        .await
        .expect("create");
    admin.create_partitions("events", 16).await.expect("expand");
    let _brokers = admin.describe_cluster().await.expect("cluster");
    let _config = admin.describe_topic_config("events").await.expect("config");
    admin
...
#[allow(dead_code)]
#[glommio::main]
async fn readme_topic_expansion() {
    let mut consumer = Consumer::new(Glommio, &brokers(), "my-app", IsolationLevel::ReadCommitted)
        .await
        .expect("consumer");
