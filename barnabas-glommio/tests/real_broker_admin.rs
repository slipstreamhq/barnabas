...
async fn admin() -> Admin {
    Admin::connect(barnabas_glommio::Glommio, &bootstrap(), "barnabas-admin")
        .await
        .expect("admin")
}
...
async fn a_topic_can_be_created_described_expanded_and_deleted() {
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn creating_an_existing_topic_is_an_error() {
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn describe_cluster_names_one_controller() {
    let mut admin = admin().await;
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn deleting_records_moves_the_log_start() {
    ...
    let records: Vec<_> = (0..5)
        .map(|i| {
            barnabas_glommio::ProducerRecord::new(None, Some(bytes::Bytes::from(format!("r{i}"))))
        })
        .collect();
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn an_assigned_consumer_is_told_when_a_topic_grows() {
    ...
    assert!(consumer.take_expansions().is_empty(), "reporting drains it");
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn a_group_picks_up_new_partitions() {
...
#[glommio::test]
#[ignore = "needs a Kafka broker on localhost:9092"]
async fn a_producer_places_keys_against_the_new_count() {
    ...
    for i in 0..8 {
        producer
            .produce(
                &topic,
                barnabas_glommio::ProducerRecord::new(
