...
fn an_idempotent_producer_round_trips() {
    run(|| async {
        ...
        let mut producer = Producer::idempotent(barnabas_glommio::Glommio, &bootstrap(), "barnabas-test")
            .await
...
fn a_committed_transaction_is_visible() {
    run(|| async {
        ...
        let mut producer = Producer::transactional(barnabas_glommio::Glommio, &bootstrap(), "barnabas-test", &unique("tid"))
            .await
...
fn a_new_producer_fences_the_old_one() {
    run(|| async {
        ...
        let mut old = Producer::transactional(barnabas_glommio::Glommio, &bootstrap(), "barnabas-test", &txn_id)
            .await
            .expect("old producer");
        ...
        let new = Producer::transactional(barnabas_glommio::Glommio, &bootstrap(), "barnabas-test", &txn_id)
            .await
            .expect("new producer");
        assert!(
