//! TLS over tokio, as a second [`Transport`](barnabas_client::Transport).
//!
...
impl barnabas_client::Transport for TokioTls {
    ...
    async fn connect(&self, addr: &str) -> io::Result<Self::Stream> {
        ...
        let tcp = <Tokio as barnabas_client::Transport>::connect(&Tokio, addr).await?;
...
pub type TlsCluster = barnabas_client::Cluster<TokioTls>;
/// An assign-only consumer over TLS.
pub type TlsConsumer = barnabas_client::Consumer<TokioTls>;
/// A producer over TLS.
pub type TlsProducer = barnabas_client::Producer<TokioTls>;
