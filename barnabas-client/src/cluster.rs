...
impl<T: Transport> Cluster<T> {
    ...
    pub async fn refresh_metadata(&mut self, topic: &str) -> Result<()> {
        ...
        for t in &resp.topics {
            let code = barnabas_core::ErrorCode(t.error_code);
            ...
            if !code.is_ok() && code.disposition() != barnabas_core::Disposition::RefreshMetadata {
    ...
    pub async fn controller_addr(&mut self) -> Result<String> {
        ...
        self.metadata
            .controller()
            .map(barnabas_core::metadata::BrokerAddr::addr)
