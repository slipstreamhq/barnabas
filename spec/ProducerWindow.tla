...
(*                                                                          *)
(* `barnabas` therefore retires only the *contiguous leading run* of          *)
(* successes and re-sends the rest, in order. `barnabas-client`'s adversarial *)
...
(*                                                                          *)
(* -- which is the same weakening that makes `barnabas`'s two ordering tests  *)
